...
export function getStats() {
  ...
  // Use EST offset (UTC-5 standard, UTC-4 daylight) — compare against EST date not UTC
  const today = db.prepare("SELECT COUNT(*) as c FROM leads WHERE date(scraped_at, '-5 hours') = date('now', '-5 hours')").get() as { c: number };
...
// ─── DELETE / PURGE ────────────────────────────────────────────────────────────
export function deleteLeadsByFilter(filter: { county?: string; source_url?: string; owner_name_contains?: string }): number {
  let sql = "DELETE FROM leads WHERE 1=1";
  const params: (string)[] = [];
  if (filter.county) { sql += " AND LOWER(county) = LOWER(?)"; params.push(filter.county); }
  if (filter.source_url) { sql += " AND source_url = ?"; params.push(filter.source_url); }
  if (filter.owner_name_contains) { sql += " AND owner_name LIKE ?"; params.push(`%${filter.owner_name_contains}%`); }
